...
import {
  ...
  Shuffle, Grid, List, Table2, LayoutGrid
} from "lucide-react";
...
export default function AwardsFeature({
  ...
  const [finalViewMode, setFinalViewMode] = useState<'carousel' | 'list'>('carousel');
  ...
  const renderActionButtons = () => {
    ...
    if (visualMode === 'intro_grid') {
      return (
        <motion.button
          ...
          className={`w-full md:w-auto px-5 py-2 md:px-6 md:py-2 rounded-full font-bold text-xs md:text-sm flex items-center justify-center gap-2 shadow-lg border transition-all ${currentProcessRank === 1
    ...
    if (visualMode === 'podium') {
      return (
        <motion.button
          ...
          className={`w-full md:w-auto px-5 py-2 md:px-6 md:py-2 rounded-full font-bold text-xs md:text-sm flex items-center justify-center gap-2 shadow-xl transition-transform hover:scale-105 border border-white/20 ${isLastCategory
    ...
    if (visualMode === 'carousel') {
      return (
        <motion.button
          initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          onClick={() => setFinalViewMode(prev => prev === 'carousel' ? 'list' : 'carousel')}
          className="w-full md:w-auto px-5 py-2 md:px-6 md:py-2 bg-white/10 hover:bg-white/20 border border-white/20 text-white rounded-full font-bold text-xs md:text-sm flex items-center justify-center gap-2 transition-all backdrop-blur-sm"
        >
          {finalViewMode === 'carousel' ? <Table2 size={16} /> : <LayoutGrid size={16} />}
          {finalViewMode === 'carousel' ? "LIHAT SEMUA DAFTAR" : "TAMPILAN CAROUSEL"}
        </motion.button>
      );
    }
  ...
  return (
    <motion.div key="awards" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center w-full max-w-[1800px] mx-auto flex-1 h-full px-2 md:px-4 relative font-sans overflow-hidden">
      ...
      {visualMode !== 'carousel' && visualMode !== 'reveal_single' && visualMode !== 'pre_roll' && currentItem && (
        <div className="mt-4 mb-2 md:mb-8 text-center shrink-0 z-10 max-w-full px-4">
          ...
          <h2 className="text-xl sm:text-2xl md:text-5xl font-black text-white drop-shadow-lg uppercase tracking-wide leading-tight break-words">
            {currentCategory}
          </h2>
        </div>
      )}
      ...
      {visualMode === 'nominations' && currentItem && (
        <div className="w-full flex-1 flex flex-col items-center justify-center relative min-h-0">
          <div className="flex items-center justify-center w-full max-w-6xl gap-2 md:gap-8 flex-1 min-h-0 h-full max-h-[50vh] md:max-h-[500px]">
              ...
              <AnimatePresence mode="wait">
                {currentNominee && (
                  <motion.div
                    ...
                    className="w-full h-full bg-gradient-to-br from-slate-900/90 to-slate-800/90 backdrop-blur-xl border border-white/20 rounded-2xl md:rounded-3xl p-4 md:p-12 flex flex-col items-center justify-center text-center shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden"
                  >
                    ...
                    <div className="relative z-10 w-full h-full flex flex-col items-center justify-center">
                      ...
                      <div className="mt-2 md:mt-6 w-full flex-1 flex items-center justify-center px-2 md:px-4 overflow-hidden">
      ...
      {visualMode === 'intro_grid' && (
        <div className="flex-1 w-full flex flex-col items-center justify-center relative overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4 w-full max-w-6xl px-4 py-4 pb-20 md:pb-4">
            {categoryItems.map((nominee, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className={`relative aspect-[3/2] md:aspect-[3/1.5] rounded-xl border ${isAlreadyWinner ? 'bg-yellow-500/20 border-yellow-400/50' : 'bg-slate-900/60 border-white/10'} backdrop-blur-md flex flex-col items-center justify-center p-2 md:p-3 shadow-xl overflow-hidden group hover:border-white/30 transition-all`}
                >
                  {isAlreadyWinner && (
      ...
      {/* 4. PRE-ROLL */}
      {visualMode === 'pre_roll' && (
        <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-xl flex flex-col items-center justify-center overflow-hidden px-4">
          ...
          >
            <div className="text-yellow-500/80 font-bold tracking-[0.3em] md:tracking-[0.5em] uppercase text-xs md:text-2xl mb-4 md:mb-8 animate-pulse">
              PEMENANGNYA ADALAH...
            </div>
            <h1 className="text-3xl sm:text-5xl md:text-8xl font-black text-white drop-shadow-[0_0_80px_rgba(255,255,255,0.5)]">
              SIAPAKAH DIA?
            </h1>
          </motion.div>
        </div>
      ...
      {visualMode === 'rolling' && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center overflow-hidden bg-slate-950">
          ...
          <div className="relative z-10 mb-8 md:mb-12 animate-pulse px-4 text-center">
            <div className="inline-flex items-center gap-3 rounded-full border border-yellow-500/30 bg-yellow-500/5 px-4 md:px-6 py-2 backdrop-blur-md shadow-[0_0_15px_-3px_rgba(234,179,8,0.3)]">
              <span className="relative flex h-3 w-3">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-yellow-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-3 w-3 bg-yellow-500"></span>
              </span>
              <span className="text-yellow-500 text-xs md:text-base font-bold uppercase tracking-[0.2em]">
                MENCARI JUARA {currentProcessRank}...
              </span>
            </div>
          </div>

          <div className="relative z-10 w-full max-w-[95vw] md:max-w-4xl h-[30vh] md:h-[50vh] flex items-center justify-center">
            <div className="w-full h-full flex items-center justify-center relative [mask-image:linear-gradient(to_bottom,transparent,black_20%,black_80%,transparent)]">
              <span
                className={`${getRollingTitleSize(rollingName)} font-black text-center break-words leading-tight tracking-tight text-transparent bg-clip-text bg-gradient-to-b from-white via-slate-200 to-slate-500 drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] scale-100 transition-transform duration-75`}
      ...
      {visualMode === 'reveal_single' && getCurrentWinner() && (
        <div className="fixed inset-0 z-50 bg-slate-900 flex flex-col items-center justify-center p-4">
          <div className={`absolute inset-0 opacity-30 blur-[100px] ${currentProcessRank === 1 ? 'bg-yellow-500/40' :
          ...
          >
            <div className="text-white/80 text-sm md:text-xl font-bold uppercase tracking-[0.3em] mb-4 text-center">
              SELAMAT KEPADA
            </div>

            <div className={`mb-6 md:mb-8 w-16 h-16 md:w-24 md:h-24 rounded-full flex items-center justify-center text-2xl md:text-5xl font-black border-4 shadow-2xl ${currentProcessRank === 1 ? 'bg-yellow-400 border-yellow-200 text-yellow-900' :
            ...
            <div className="text-center w-full max-w-7xl px-2 md:px-4">
              <div className="w-full h-full flex items-center justify-center">
                <span
                  className={`${getRevealTitleSize(getCurrentWinner()?.company || "")} font-bold text-white drop-shadow-[0_0_20px_rgba(255,255,255,0.45)] mb-2 break-words leading-none`}
                >
            ...
            <motion.button
              ...
              className="mt-8 px-6 py-3 md:px-8 bg-white text-slate-900 rounded-full font-bold text-xs md:text-base hover:scale-105 hover:bg-slate-200 transition-all shadow-[0_0_30px_rgba(255,255,255,0.3)] flex items-center justify-center gap-2 z-50 border border-transparent hover:border-white/50"
            >
      ...
      {visualMode === 'podium' && (
        <div className="w-full flex-1 flex flex-col items-center justify-center relative min-h-0 py-4">
          <div className="flex flex-col items-center w-full h-full max-w-7xl mx-auto justify-end md:justify-center pt-10">
      ...
      {visualMode === 'carousel' && (
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center w-full flex-1 justify-center relative min-h-0">
          ...
          {/* OPSI 1: CAROUSEL MODE */}
          {finalViewMode === 'carousel' && (
            <>
              <div
                className="w-full max-w-[1600px] mx-auto flex items-center justify-between px-1 md:px-4 mt-2 md:mt-4 relative z-20 flex-1 min-h-0"
              ...
              >
                ...
                <div className="flex-1 flex justify-center items-center h-full px-2 md:px-4 overflow-hidden relative">
                  <AnimatePresence mode="wait">
                    {uniqueCategories.map((categoryName, index) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="flex flex-col items-center w-full max-w-6xl bg-slate-900/40 border border-white/5 rounded-[1.5rem] md:rounded-[3rem] p-2 md:p-12 shadow-2xl backdrop-blur-md h-full justify-center max-h-[80vh] min-h-[300px]"
                        >
                          <div className="text-center mb-4 md:mb-8 mt-4">
                            <h3 className="text-white/60 text-[10px] md:text-sm font-bold uppercase tracking-widest mb-1">Pemenang Kategori</h3>
                            <h2 className="text-lg md:text-4xl font-black text-white">{categoryName}</h2>
                          </div>
              ...
              <div className="flex items-center gap-4 mt-2 md:mt-6 shrink-0 pb-4 md:pb-8 flex-col md:flex-row">
          ...
          {/* OPSI 2: SUMMARY LIST VIEW */}
          {finalViewMode === 'list' && (
            <div className="w-full h-full flex flex-col items-center relative z-20 flex-1 min-h-0 overflow-hidden">
              <div className="w-full max-w-5xl flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none'] p-4 space-y-6 md:space-y-8 pb-24">
                <div className="text-center mb-8">
                  <h2 className="text-2xl md:text-4xl font-black text-white mb-2">DAFTAR PEMENANG LENGKAP</h2>
                  <p className="text-slate-400 text-xs md:text-sm">Selamat kepada seluruh pemenang awards tahun ini.</p>
                </div>

                {uniqueCategories.map((cat, idx) => {
                  const catItems = mergedAwardWinners.filter(w => w.category === cat).sort((a, b) => a.rank - b.rank);
                  return (
                    <motion.div
                      initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}
                      key={cat} className="bg-slate-900/60 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-sm"
                    >
                      <div className="bg-white/5 px-4 py-3 border-b border-white/5 flex items-center justify-between">
                        <h3 className="font-bold text-white text-sm md:text-lg">{cat}</h3>
                        <Medal size={16} className="text-yellow-500" />
                      </div>
                      <div className="p-2 md:p-4 grid grid-cols-1 md:grid-cols-3 gap-2">
                        {catItems.map((winner) => (
                          <div key={winner.id} className={`flex items-center gap-3 p-3 rounded-xl border ${winner.rank === 1 ? 'bg-gradient-to-r from-yellow-500/10 to-transparent border-yellow-500/20' : 'bg-slate-800/30 border-white/5'}`}>
                            <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center font-bold text-sm md:text-base border-2 shadow-lg shrink-0 ${winner.rank === 1 ? 'bg-yellow-400 border-yellow-600 text-yellow-900' :
                              winner.rank === 2 ? 'bg-slate-300 border-slate-400 text-slate-900' :
                                'bg-orange-400 border-orange-600 text-orange-900'
                              }`}>
                              {winner.rank}
                            </div>
                            <div className="min-w-0">
                              <div className="text-white font-bold text-xs md:text-sm truncate leading-tight">{winner.company}</div>
                              <div className="text-slate-500 text-[10px] md:text-xs">Winner Rank {winner.rank}</div>
                            </div>
                            {winner.rank === 1 && <Crown size={14} className="ml-auto text-yellow-500" />}
                          </div>
                        ))}
                      </div>
                    </motion.div>
...
function PodiumContent({
  ...
  return (
    <>
      <div className="flex flex-nowrap items-end justify-center gap-1 sm:gap-3 md:gap-4 w-full h-auto min-h-[250px] md:min-h-[40vh] flex-1 shrink-0 px-0 md:px-4 pb-12 md:pb-20">

        {/* RANK 2 */}
        {rank2 && (
          <motion.div
            initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className="flex flex-col items-center order-1 relative w-[30%] max-w-[280px]"
          >
            <div className="w-full h-[18vh] md:h-[30vh] bg-gradient-to-t from-slate-700/90 to-slate-600/30 border-t-2 border-x-2 border-slate-400/50 rounded-t-lg md:rounded-t-2xl relative flex flex-col items-center justify-between py-2 md:py-4 backdrop-blur-md shadow-lg">
              <div className="w-full flex-1 flex items-center justify-center px-0.5 md:px-1 mb-1 z-10 min-h-0 overflow-hidden">
                <div className="text-white font-bold leading-tight w-full drop-shadow-md text-center h-full flex items-center justify-center px-0.5">
                  {isCarousel
                    ? <AutoFitTextCarousel text={rank2.company} />
                    : <AutoFitText text={rank2.company} />}
                </div>
              </div>
              <div className="text-xl md:text-6xl font-black text-slate-300 opacity-60 pb-1 shrink-0">2</div>
            </div>
            <div className="w-6 h-6 md:w-16 md:h-16 rounded-full bg-slate-300 flex items-center justify-center text-slate-900 font-bold border md:border-4 border-slate-600 shadow-xl z-10 -mt-3 md:-mt-8 text-[10px] md:text-base">
              <Medal className="text-slate-900 w-3 h-3 md:w-8 md:h-8 hidden md:block" />
        ...
        {rank1 && (
          <motion.div
            ...
            className="flex flex-col items-center order-2 z-10 relative w-[38%] max-w-[360px]"
          >
            {/* PERBAIKAN UTAMA:
                Mahkota (Crown) sekarang menggunakan 'relative' dan 'negative margin-bottom (-mb)'.
                Ini membuatnya tetap memakan ruang vertikal di layout (mendorong elemen atas),
                tapi secara visual terlihat menempel di atas kotak.
                Ini mencegah tabrakan dengan teks di atasnya.
            */}
            <div className="relative z-30 -mb-6 sm:-mb-8 md:-mb-14 animate-bounce">
              <Crown className="text-yellow-400 fill-yellow-400/20 w-8 h-8 md:w-20 md:h-20 drop-shadow-[0_0_20px_rgba(234,179,8,0.8)]" />
            </div>

            <div className="w-full h-[24vh] md:h-[42vh] bg-gradient-to-t from-yellow-600/90 to-yellow-500/40 border-t-2 border-x-2 border-yellow-400/60 rounded-t-xl md:rounded-t-3xl relative flex flex-col items-center justify-between py-2 md:py-4 backdrop-blur-md shadow-[0_0_80px_rgba(234,179,8,0.4)]">
              <div className="w-full flex-1 flex items-center justify-center px-0.5 md:px-1 mb-1 z-20 pt-2 md:pt-4 min-h-0 overflow-hidden">
                <div className="text-white font-black drop-shadow-[0_4px_4px_rgba(0,0,0,0.8)] w-full text-center h-full flex items-center justify-center px-0.5">
                  {isCarousel
                    ? <AutoFitTextCarousel text={rank1.company} />
                    : <AutoFitText text={rank1.company} />}
                </div>
              </div>
              <div className="text-3xl md:text-8xl font-black text-yellow-300 opacity-60 pb-1 shrink-0">1</div>
            </div>
            <div className="w-10 h-10 md:w-24 md:h-24 rounded-full bg-yellow-400 flex items-center justify-center text-yellow-900 font-bold border-2 md:border-4 border-yellow-600 shadow-2xl z-20 -mt-5 md:-mt-12 scale-110 text-xs md:text-xl">
              <Trophy className="w-5 h-5 md:w-12 md:h-12 hidden md:block" />
        ...
        {rank3 && (
          <motion.div
            ...
            className="flex flex-col items-center order-3 relative w-[30%] max-w-[280px]"
          >
            <div className="w-full h-[14vh] md:h-[25vh] bg-gradient-to-t from-orange-800/90 to-orange-700/30 border-t-2 border-x-2 border-orange-500/50 rounded-t-lg md:rounded-t-2xl relative flex flex-col items-center justify-between py-2 md:py-4 backdrop-blur-md shadow-lg">
              <div className="w-full flex-1 flex items-center justify-center px-0.5 md:px-1 mb-1 z-10 min-h-0 overflow-hidden">
                <div className="text-white font-bold leading-tight w-full drop-shadow-md text-center h-full flex items-center justify-center px-0.5">
                  {isCarousel
              ...
              <div className="text-xl md:text-6xl font-black text-orange-400 opacity-60 pb-1 shrink-0">3</div>
            </div>
            <div className="w-6 h-6 md:w-16 md:h-16 rounded-full bg-orange-700 flex items-center justify-center text-orange-100 font-bold border md:border-4 border-orange-900 shadow-xl z-10 -mt-3 md:-mt-8 text-[10px] md:text-base">
              <Award className="text-orange-100 w-3 h-3 md:w-8 md:h-8 hidden md:block" />
...
const AutoFitText = ({ text }: { text: string }) => {
  const len = text.length;
  // Di Mobile (layar kecil), font sangat kecil agar muat
  ...
  return (
    <div className="w-full h-full flex items-center justify-center overflow-hidden p-0.5 md:p-1">
      <span
...
const AutoFitTextCarousel = ({ text }: { text: string }) => {
  const len = text.length;
  let sizeClass = "text-[9px] md:text-base leading-tight font-semibold";

  if (len > 15) sizeClass = "text-[8px] md:text-sm leading-none font-semibold";
  if (len > 25) sizeClass = "text-[7px] md:text-xs leading-none font-medium tracking-tight";
  if (len > 35) sizeClass = "text-[6px] md:text-[10px] leading-[1] font-medium tracking-tighter";
  if (len > 50) sizeClass = "text-[5px] md:text-[9px] leading-[0.95] font-normal tracking-tighter";
...
const AutoFitTextGrid = ({ text }: { text: string }) => {
  ...
  if (len > 15) sizeClass = "text-[8px] md:text-sm leading-tight font-semibold";
  if (len > 25) sizeClass = "text-[7px] md:text-xs leading-none font-medium";
  if (len > 35) sizeClass = "text-[6px] md:text-[11px] leading-none font-medium";
  if (len > 50) sizeClass = "text-[5px] md:text-[10px] leading-none font-normal tracking-tight";
...
const AutoFitTextHero = ({ text }: { text: string }) => {
  const len = text.length;
  let sizeClass = "text-xl md:text-4xl leading-tight font-extrabold";
  ...
  return (
    <div className="w-full h-full flex items-center justify-center overflow-hidden px-1 md:px-4">
      <span
